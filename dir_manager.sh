...
MONTHLY_FILENAME="`date +%Y%m`--arr_stack_backup${BACKUP_SUFFIX}"
...
MFILE="${FINAL_DESTINATION}/monthly/${MONTHLY_FILENAME}"
...
echo -e "${GREEN}OK${NC} -- Created ./last ./daily ./weekly ./monthly directories"
...
echo -e "${GREEN}OK${NC} -- File compressed directly to destination"
echo -e "${GREEN}OK${NC} -- Created ${CYAN}'$LAST_FILENAME'${NC} ($BACKUP_SIZE)"
echo ""

# Creating a hardlink of the file to each directory.
ln -vf "${FILE}" "${DFILE}"
echo -e "${GREEN}OK${NC} -- Hardlink created from latest to daily folder"
ln -vf "${FILE}" "${WFILE}"
echo -e "${GREEN}OK${NC} -- Hardlink created from latest to weekly folder"
ln -vf "${FILE}" "${MFILE}"
echo -e "${GREEN}OK${NC} -- Hardlink created from latest to monthly folder"
...
if [ "${BACKUP_LATEST_TYPE}" = "symlink" -o "${BACKUP_LATEST_TYPE}" = "hardlink"  ]; then
    ...
    echo -e "${GREEN}OK${NC} -- Symlink for latest backup in last directory"
    ln "${LATEST_LN_ARG}" -vf "${DAILY_FILENAME}" "${FINAL_DESTINATION}/daily/arr_stack_backup-latest${BACKUP_SUFFIX}"
    echo -e "${GREEN}OK${NC} -- Symlink for latest backup in daily directory"
    ln "${LATEST_LN_ARG}" -vf "${WEEKLY_FILENAME}" "${FINAL_DESTINATION}/weekly/arr_stack_backup-latest${BACKUP_SUFFIX}"
    echo -e "${GREEN}OK${NC} -- Symlink for latest backup in weekly directory"
    ln "${LATEST_LN_ARG}" -vf "${MONTHLY_FILENAME}" "${FINAL_DESTINATION}/monthly/arr_stack_backup-latest${BACKUP_SUFFIX}"
    echo -e "${GREEN}OK${NC} -- Symlink for latest backup in monthly directory"
else # [ "${BACKUP_LATEST_TYPE}" = "none"  ]
    echo -e "${YELLOW}OK${NC} -- No symlink created as set by user"
fi

#Clean old files
echo -e "${CYAN} --Cleaning older backups for the Arr_Stack_Suite-- ${NC}"
...
echo -e "${GREEN}OK${NC} -- Oldest backups cleaned from system"